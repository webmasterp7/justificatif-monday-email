  ...
  return JSON.stringify(
    {
      ...
      instructions: [
        ...
        'Grouping rule: attachments may share one group only when all three conditions are true: same provider, same service, and different document kinds.',
        'Document kinds are invoice, receipt, payment_proof, supporting_document, or other. Classify each attachment in groupingEvidence with provider, service, and documentKind.',
        'Two invoices must always be separate groups, even if they are in the same email, have the same course/project/theme, or are both mentioned as related.',
        'An invoice may be grouped with a receipt or payment_proof only when the provider and service are the same.',
        'Supporting documents such as participant lists or attendance sheets must be separate groups unless they clearly have the same provider and same service as a non-supporting document and have a different document kind.',
        'When the grouping rule cannot be proven, prefer separate Attention-ready groups over merging. Use one fallback group containing all attachments only when the files are unreadable and distinct groups cannot be identified at all.',
        ...
        receiptGroups: [
          {
            ...
            groupingExplanation: {
              status: 'confident | uncertain | missing',
              value: 'why the grouping rule is satisfied, or why this is a separate Attention-ready group',
              reason: 'required when status is uncertain or missing',
            },
            ...
            groupingEvidence: [
              {
                ...
                reason: 'short evidence for provider/service/documentKind',
              },
            ],
            typeDeFacture: {
              status: 'confident | uncertain | missing',
