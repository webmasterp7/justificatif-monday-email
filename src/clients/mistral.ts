  ...
  return JSON.stringify(
    {
      ...
      instructions: [
        ...
        'Group by transaction/expense, not by email: an invoice and receipt/proof of payment for the same amount/vendor/reference/service should be in the same group.',
        'Separate unrelated supplier invoices/receipts into separate groups when vendors, amounts, references, services, or transaction context differ.',
        'Supporting documents such as participant lists or attendance sheets must not force separate invoices into one group; assign them only when they clearly support that same transaction, otherwise keep a separate uncertain group.',
        'When grouping is uncertain, return create_items with one fallback group containing all attachments, low confidence, and uncertain groupingExplanation only if you cannot identify distinct transactions (do not lose any attachment).',
